...
//~~~~~~~~~~~~~~~Requete~~~~~~~~~~~~~~~~//
$sqlDatas = mysql_query("SELECT `$champCodePostal`, `$champPays` FROM `$tableUtilisee` WHERE `$champWaiting` = '0' AND `$champSuspend` = '0' AND `$champSuspendAdmin` = '0' AND ".time()." < `$champExpired` ORDER BY `$champCodePostal`");
if (!$sqlDatas) {
		...
		if (trim(strtolower($row[$champPays])) == 'france') {
			...
			if (substr($row[$champCodePostal], 0, 2) == '20') {
				if (substr($row[$champCodePostal], 0, 3) == '201' || substr($row[$champCodePostal], 0, 3) == '200') {
					$tabDep['2A']++;
				} else if (substr($row[$champCodePostal], 0, 3) == '202') {
					$tabDep['2B']++;
				}
			} else {
