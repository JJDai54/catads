...
xoops_cp_header();
//ajout CPascalWeb - menu admin
	if ( !is_readable(XOOPS_ROOT_PATH . "/Frameworks/art/functions.admin.php"))	{
		catads_admin_menu(6, _AM_CATADS_IMPORTMANAGE);
	} else {
		include_once XOOPS_ROOT_PATH.'/Frameworks/art/functions.admin.php';
//		loadModuleAdminMenu (6, _AM_CATADS_IMPORTMANAGE);
	}
//catads_admin_menu(6, _AM_CATADS_IMPORTMANAGE);
//fin
...
//ajout fonction CPascalWeb - 17 septembre posibilité de suspendre ou de réactivé une annonce (suspendadmin) et signalement d'une annonce frauduleuse (signalementannonce) 
//ajout CPascalWeb - 12 novembre 2010 - option tel portable et modif CPascalWeb - 18 mai 2011 supp expired_mail_send plus besoin envoi mail automatique	
					   $insert = $xoopsDB->queryF("INSERT INTO ".$xoopsDB->prefix("catads_ads")." (
                        ads_id, cat_id, ads_title, ads_type, ads_desc, price, monnaie, price_option, email, uid, phone, phoneportable, pays, region, departement, town, codpost, created, published, expired, view, notify_pub, poster_ip, contact_mode, countpub, suspend, suspendadmin, signalementannonce, waiting, photo0, photo1, photo2, photo3, photo4, photo5, thumb) VALUES ('".$donnees['lid']."','".$donnees['cid']."','".$title."','".$donnees['type']."','".$desctext."','".$donnees['price']."','Euros', '$typeprice','".$donnees['email']."','".$donnees['usid']."','".$donnees['tel']."','FRANCE', '00', '00','".$donnees['town']."','','".$donnees['date']."','".$donnees['date']."','".$expire."', '1', '".$donnees['hits']."', '1', '0', '0', '0', '0', '".$donnees['status']."', '".$photo0."','".$photo1."','".$photo2."', '".$photo3."','".$photo4."','".$photo5."','')");
                ...
                //Inserer les donnees des categories 
                ...
                //Insertion de differents types d'annonces
                 ...
                 while($dossier=@readdir($racine))
                        {
                                if(!in_array($dossier, Array("..", ".")))
                                {
                                        ...
                                        if(copy($fileCopy, XOOPS_ROOT_PATH."/uploads/".$xoopsModule->dirname()."/images/annonces/original/".$dossier."")) $copy = 1;
                                        $img = "img_".$image[2];
                                        rename( XOOPS_ROOT_PATH."/uploads/".$xoopsModule->dirname()."/images/annonces/original/".$dossier."" , XOOPS_ROOT_PATH."/uploads/".$xoopsModule->dirname()."/images/annonces/original/".$img."" ) ;
                                        ...
                                        if ( $copy == 1 ){
                                                //Copie reussie
                                                $copy = "<img src='".XOOPS_URL."/modules/".$xoopsModule->dirname()."/images/icon/enligne.png'>";
                                        } else {
                                                //Copie echoué
                                                $copy = "<img src='".XOOPS_URL."/modules/".$xoopsModule->dirname()."/images/icon/avertissement.png'>";
                                        }
                                        if ( $resize == 1 ){
                                                //Creation reussie
                                                $resize = "<img src='".XOOPS_URL."/modules/".$xoopsModule->dirname()."/images/icon/enligne.png'>";
                                        } elseif ( $resize == 2 ) {
                                                //Thumb deja existant dans la db
                                                $resize = "<img src='".XOOPS_URL."/modules/".$xoopsModule->dirname()."/images/icon/attention_pf.png'>";
                                        } else {
                                                //Thumb echoué
                                                $resize = "<img src='".XOOPS_URL."/modules/".$xoopsModule->dirname()."/images/icon/avertissement.png'>";
                                        }
